...
describe('test.data-type-order.js', () => {
  it('Orders different data types correctly', async () => {
    const db = context.db;
    ...
    resp1.docs.should.deep.equal([
      {_id: '4'},
      {_id: '3'},
      {_id: '2'},
      {_id: '1'},
    ]);
    const resp2 = await db.find({
      selector: {
        foo: {$gt: 0}
      },
      fields: ['_id'],
    });
    console.log(JSON.stringify(resp2.docs, null, 2));
    resp2.docs.should.deep.equal([
      {_id: '6'},
      {_id: '7'},
      {_id: '8'},
      {_id: '9'},
  ...
  it.skip('Orders different data types correctly, including objects', async () => {
    const db = context.db;
    await db.createIndex({
      index: {
          fields: ['foo']
  ...
  it.skip('Orders min numbers correctly', async () => {
    const db = context.db;
    await db.createIndex({
      index: {
        fields: ['foo']
