...
function HttpPouch(opts, callback) {
  ...
  async function fetchJSON(url, options, callback) {
  ...
  async function setup() {
    ...
    try {
      setupPromise = await fetchJSON(dbUrl);
    } catch (err) {
      try {
        if (err.status && err.status === 404) {
          explainError(404, 'PouchDB is just detecting if the remote exists.');
          // Doesnt exist, create it
          setupPromise = await fetchJSON(dbUrl, {method: 'PUT'});
        } else {
          throw err;
        }
      } catch (err) {
  ...
  api.bulkGet = coreAdapterFun('bulkGet', function (opts, callback) {
    ...
    async function doBulkGet(cb) {
      ...
      try {
        let result = await fetchJSON(genDBUrl(host, '_bulk_get' + paramsToStr(params)), {
          method: 'POST',
  ...
  api.remove = adapterFun('remove', async function (docOrId, optsOrRev, opts, cb) {
    ...
    try {
      await fetchJSON(url, {method: 'DELETE'}, cb);
    } catch (err) {
      cb(err);
    }
  });
  ...
  api.removeAttachment =  adapterFun('removeAttachment', async function (
    ...
    try {
      await fetchJSON(url, {method: 'DELETE'}, callback);
    } catch (error) {
      callback(error);
    }
  });
  ...
  api.putAttachment = adapterFun('putAttachment', async function (docId, attachmentId,
                                                            rev, blob,
                                                            type, callback) {
  ...
  api._changes = async function (opts) {
    ...
    var fetchData = async function (since, callback) {
      if (opts.aborted) {
        return;
      }
      params.since = since;
      ...
      try {
        await setup();
        return await fetchJSON(url, fetchOpts, callback);
      } catch (err) {
        callback(err);
      }
    };
    ...
    var fetched = function (err, res) {
      ...
      if ((opts.continuous && !(limit && leftToFetch <= 0)) || !finished) {
        ...
        nextTick(async function () { await fetchData(lastFetchedSeq, fetched); });
      } else {
    ...
    await fetchData(opts.since || 0, fetched);
  ...
  api.revsDiff = adapterFun('revsDiff', async function (req, opts, callback) {
    ...
    try {
      await fetchJSON(genDBUrl(host, '_revs_diff'), {
        method: 'POST',
        body: JSON.stringify(req)
      }, callback);
    } catch (error) {
