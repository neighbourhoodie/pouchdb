...
function purgeAttachments(doc, revs) {
  ...
  for (let k in doc.attachments) {
    ...
    // TODO: there could be inconsistencies where an attachment refers to a rev that doens't have refer back to the
    //   attachment in its `revs` entry. we could handle that here and remove those ghost references, and thus be able
    //   to confidently remove the attachment below if there are no other revs referring to that attachment.
