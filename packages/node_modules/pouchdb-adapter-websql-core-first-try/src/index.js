...
function fetchAttachmentsIfNecessary(doc, opts, api, txn, cb) {
  ...
  function checkDone() {
    console.log('fetchAttachments checkDone()', numDone)
    if (++numDone === attachments.length && cb) {
      console.log('  calling cb()', cb.toString())
      cb();
    }
  }
  ...
  function fetchAttachment(doc, att) {
    ...
    api._getAttachment(doc._id, att, attObj, attOpts, function (_, data) {
      console.log('fetchAttachment', doc._id, att)
...
function WebSqlPouch(opts, callback) {
  ...
  db.executeSql = function(sql, params = [], successCb, errorCb) {
    ...
    try {
      ...
      if (sql.startsWith('select') || sql.startsWith('SELECT')) {
        ...
        successCb && successCb(db, result)
    } else {
        const meta = stmt.run(...params)
        console.log('  meta', meta)
        successCb && successCb(db, meta)
      }
    } catch(error) {
      console.log('  error: ', error)
      if (errorCb) {
        const errorCbResult = errorCb(db, error)
        if (errorCbResult !== false) {
          throw error
        }
      }
    }
  ...
  db.transaction = function (txnCallback, errorCallback, successCallback) {
    // this._txtQueue.push({ txnCallback, errorCallback, successCallback})
    // wait
  ...
  function onGetInstanceId() {
    ...
    // callback() start task queue
  }

  function onGetVersion(tx, dbVersion) {
    console.log('> onGetVersion: ', dbVersion)
    if (dbVersion === 0) {
      // initial schema
      ...
      var setupDone = function () {
        ...
        tx.executeSql(sql, [], function (tx, result) {
          console.log('setupDone SELECT dbid FORM META_STORE', result)
  ...
  api._get = function (id, opts, callback) {
    ...
    tx.executeSql(sql, sqlArgs, function (a, results) {
      console.log('api._get results', JSON.stringify(results), results.length)
      if (!results.length) {
  ...
  api._changes = function (opts) {
    opts = clone(opts);
    console.log('> api._changes')
    if (opts.continuous) {
      var id = api._name + ':' + uuid();
      console.log('  continuous', api._name, id)
      ...
      db.readTransaction(function (tx) {
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          function reportChange(change) {
            console.log('  > reportChange', change)
            return function () {
              console.log('> reportChange opts.onChange', change, opts.onChange.toString())
              nextTick(() => opts.onChange(change));
            };
          }
        ...
        console.log('  txn errorCb', error)
        const fn = websqlError(opts.complete)
        fn(error)
      }, function () {
        console.log('  txn.success', results, lastSeq)
        if (!opts.continuous) {
          opts.complete(null, {
            results: results,
            last_seq: lastSeq
  ...
  api._getAttachment = function (docId, attachId, attachment, opts, callback) {
    ...
    tx.executeSql(sql, [digest], function (tx, result) {
      ...
      console.log('api._getAttachment', item, opts.binary, callback.toString())
      callback(null, res);
    });
  };
