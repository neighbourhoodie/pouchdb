...
function tryCatchInChangeListener(self, change, pending, lastSeq) {
  ...
  console.log('> tryCatchInChangeLIstener')
  try {
    self.emit('change', change, pending, lastSeq);
    console.log('  emitted', self)
  } catch (e) {
    console.log('  error: ', e)
...
class Changes extends EE {
  constructor(db, opts, callback) {
    ...
    if (!db.taskqueue.isReady) {
      console.log('task queue not ready')
      db.taskqueue.addTask((failed) => {
        if (failed) {
          console.log('  adding to task queue failed')
          opts.complete(failed);
        } else if (this.isCancelled) {
          console.log('  picked off the task queue, but has been cancelled since')
          this.emit('cancel');
        } else {
          console.log('  picked off the task queue')
          this.validateChanges(opts);
        }
      });
    } else {
      console.log('task queue is ready, let’s just go')
      this.validateChanges(opts);
    }
  }
  ...
  doChanges(opts) {
    ...
    console.log('doChanges, call api._changes', opts)
